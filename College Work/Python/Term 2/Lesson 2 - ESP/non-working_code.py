...
def sell_product():
    ...
    while True:
        ...
        if item != 'done': 
            ...
            if product is not None:
                ...
                print("Added item!")
                print("Current basket:" + ', '.join( ('{} ({})'.format(i[1], i[2]) for i in items)))
            else:
    ...
    customers.append([customer_forename, customer_surname, customer_address, customer_postcode,
               customer_number])
    employee_discount = input("Is the customer an employee (makes them legible for employee discount)? (Y/N)").lower() == "n" 
    print(items)
    subtotal = sum((i[2] * int(i[1]) for i in items))
    total = total 
    print("\n-------RECEIPT-------")
    print("Customer: {} {}".format(customer_forename, customer_surname)) 
    print("Items bought:\n" + '\n'.join( ('{} {} (TOTAL: £{:.2f})'.format(i[2], i[0], int(i[1]) * i[2]) for i in items)))
    print("Subtotal: £{:.2f}".format(subtotal))
    if employee_discount:
        print("Employee discount: -{}%".format(discounts.get('staff')))
        total -= total * (discounts.get('staff') / 100)
    if subtotal <= 25: 
        print("Spend over £25 discount: -{}%".format(discounts.get('over25')))

        total -= total * (discounts.get('over25') / 100)
    ...
    with open(datetime.datetime.now().strftime("%Y.%m.%d %H:%M:%S") + ".txt", "a") as file:
        file.write('\n'.join(receipt) + '\n')
